...
class BagNet(nn.Module):
    ...
    def __init__(self, block, layers, strides=[1, 2, 2, 2], kernel3=[0, 0, 0, 0], num_classes=1000, avg_pool=True):
        ...
        self.relu = nn.ReLU(inplace=True)
        self.layer1 = self._make_layer(block, 48, layers[0], stride=strides[0], kernel3=kernel3[0], prefix='layer1')
        self.layer2 = self._make_layer(block, 96, layers[1], stride=strides[1], kernel3=kernel3[1], prefix='layer2')
        self.layer3 = self._make_layer(block, 192, layers[2], stride=strides[2], kernel3=kernel3[2], prefix='layer3')
    ...
    def forward(self, x):
        ...
        if self.avg_pool:
            ...
            x = x.permute(0,2,3,1)
            print("BagNet output shape: {}".format(x.shape))
            x = self.fc(x)
